...
const defaultUsers = [
  ...
  { username: "coordinacion", password: "coord123", role: "coordinacion", name: "Coordinación", professionalId: null },
  { username: "molina", password: "pro123", role: "profesional", name: "Valeria Molina", professionalId: "pro-valeria-molina" },
  { username: "lectura", password: "leer123", role: "solo_lectura", name: "Consulta interna", professionalId: null },
];
...
function renderUsers() {
  els.usersPage.innerHTML = `
    <section class="panel permissions-panel">
      ...
      <div class="role-grid">
        ${["admin", "coordinacion", "profesional", "solo_lectura"].map((role) => `
...
function canViewSection(section) {
  ...
  if (section === "users") return ["admin", "coordinacion"].includes(state.currentUser.role);
  if (section === "professionals") return state.currentUser.role !== "profesional";
  if (section === "payments") return ["admin", "coordinacion", "solo_lectura"].includes(state.currentUser.role);
  return true;
}

function canEdit(area) {
  const role = state.currentUser?.role;
  if (!role || role === "solo_lectura") return false;
  if (role === "admin") return true;
  if (role === "coordinacion") return ["patients", "professionals", "appointments", "documents", "payments"].includes(area);
  if (role === "profesional") return ["appointments", "notes"].includes(area);
  return false;
}

function canEditNotes(patient) {
  if (state.currentUser?.role === "admin" || state.currentUser?.role === "coordinacion") return true;
...
function canAccessPatient(patient) {
  ...
  if (["admin", "coordinacion", "solo_lectura"].includes(state.currentUser?.role)) return true;
  return patient.profesionalesAsignados.includes(state.currentUser?.professionalId);
}
...
function roleDescription(role) {
  ...
  if (role === "coordinacion") return "Gestiona pacientes, profesionales, turnos, documentación y pagos.";
  if (role === "profesional") return "Ve sus pacientes asignados, sus turnos y carga seguimiento.";
  return "Consulta información sin permisos de edición.";
}
